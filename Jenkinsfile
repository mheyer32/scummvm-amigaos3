...
def buildStep(ext) {
	...
	try{
		stage("Building ${ext}...") {
			...
			sh "cd ${env.WORKSPACE}/ && CC=\"ccache ${ext}-gcc --sysroot=/opt/toolchains/${ext}/\" CPP=\"ccache ${ext}-cpp --sysroot=/opt/toolchains/${ext}/\" CXX=\"ccache ${ext}-g++ --sysroot=/opt/toolchains/${ext}/\" ${env.WORKSPACE}/configure --host=${ext} --with-sdl-prefix=/opt/toolchains/${ext}/ --disable-all-engines --enable-engine=scumm,scumm-7-8 --disable-mt32emu --enable-release --disable-hq-scalers --disable-mad --prefix=/opt/toolchains/${ext}/ --with-amiga-prefix=${env.WORKSPACE}/publishing/deploy/scummvm/${ext}/"
			if (!env.CHANGE_ID) {
				sh "mkdir -p ${env.WORKSPACE}/publishing/deploy/scummvm/${ext}/"
      }
			sh "cd ${env.WORKSPACE}/ && make -j8 clean"
			
			sh "cd ${env.WORKSPACE}/ && make -j8 "

			sh "cd ${env.WORKSPACE}/ && make amigaos3dist -j8"
