...
def buildStep(ext) {
	...
	try{
		stage("Building ${ext}...") {
			...
			sh "cd ${env.WORKSPACE}/ && CC=\"ccache ${ext}-gcc\" CPP=\"ccache ${ext}-cpp\" CXX=\"ccache ${ext}-g++\" ${env.WORKSPACE}/configure --host=${ext} --with-sdl-prefix=/opt/toolchains/${ext}/ --disable-all-engines --enable-engine=scumm,scumm-7-8 --disable-mt32emu --enable-release --disable-hq-scalers --disable-mad --prefix=/opt/toolchains/${ext}/ --with-amiga-prefix=${env.WORKSPACE}/publishing/deploy/scummvm/${ext}/"
			if (!env.CHANGE_ID) {
